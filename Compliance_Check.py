...
def generate_simulated_data(n_transactions=1000):
    ...
    data = {
        ...
        "patient_name": [f"patient_{i}" for i in range(n_transactions)],
        "ssn": [f"{random.randint(100,999)}-{random.randint(10,99)}-{random.randint(1000,9999)}" for _ in range(n_transactions)],
        "credit_card": [f"{random.randint(4000,4999)}-{random.randint(1000,9999)}-{random.randint(1000,9999)}-{random.randint(1000,9999)}" for _ in range(n_transactions)]
    }
...
# Smart Contract Compliance Check

def smart_contract_compliance_check(df):
    restricted_countries = ['IR', 'RU', 'CN']
    non_compliant = df[df['country'].isin(restricted_countries)]
    return non_compliant, df[~df['country'].isin(restricted_countries)]

def check_compliance(df):
    hippa_violations = df[df['patient_name'].notnull() & df['ssn'].notnull()]
    gdpr_violations = df[df['country'].isin(['DE', 'FR']) & df['ssn'].notnull()]
    pci_violations = df[df['credit_card'].str.contains("[0-9]{4}-[0-9]{4}-[0-9]{4}-[0-9]{4}")]
    return hippa_violations, gdpr_violations, pci_violations
...
def run_dashboard(blockchain, df, insights, anomalies, decrypted_data, non_compliant, hippa, gdpr, pci):
    ...
    elif view == "Non-Compliant Transactions":
        st.subheader("🚫 Transactions Failing Compliance Check")
        st.dataframe(non_compliant)

    elif view == "HIPAA Violations":
        st.subheader("🛡 HIPAA Violations")
        st.dataframe(hippa)

    elif view == "GDPR Violations":
        st.subheader("🇪🇺 GDPR Violations")
        st.dataframe(gdpr)

    elif view == "PCI-DSS Violations":
        st.subheader("💳 PCI-DSS Violations")
        st.dataframe(pci)
...
def main():
    ...
    non_compliant, compliant_data = smart_contract_compliance_check(anonymized_data)
    hippa, gdpr, pci = check_compliance(compliant_data)
    qkd_key = simulate_qkd_key()
    encrypted_data = encrypt_data(compliant_data.to_dict(), qkd_key)
    ...
    run_dashboard(blockchain, analyzed_df, insights, anomalies, decrypted_data, non_compliant, hippa, gdpr, pci)
